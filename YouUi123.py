...
class YOLOv5GUI(QMainWindow):
    ...
    def detect(self):
        if not self.image_files:
            logging.warning('No image files selected.')
            return

        selected_model_path = os.path.join("pt", self.model_combobox.currentText())
        logging.info(f'Selected model path: {selected_model_path}')

        try:
            # Load model
            if self.model is None:
                logging.info('Loading YOLOv5 model...')
                self.model = torch.hub.load('ultralytics/yolov5', 'custom', path=selected_model_path)
                self.model.to('cpu')
            ...
            for i, img_path in enumerate(self.image_files):
                ...
                try:
                    ...
                    logging.info(f'Inference result saved to: {inferred_image_path}')
            ...
            self.append_to_info_text(f"Error during detection: {str(e)}")
